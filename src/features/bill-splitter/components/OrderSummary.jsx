...
import { calculateRemainingTotal } from "../utils/calculations";
...
export function OrderSummary({
  ...
  const remainingTotal = useMemo(() => {
    return calculateRemainingTotal(items, paidItems);
  }, [items, paidItems]);
  ...
  const handleConfirmPayment = () => {
    const finalAmount = mode === "all" ? remainingTotal : totalToPay;
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        if (onPaymentSuccess) onPaymentSuccess(finalAmount);
      }, 1500);
    }, 2000);
  };
  ...
  return (
    <>
      ...
      <ConfirmPaymentModal
        ...
        title={
          mode === "all"
            ? "Pagar resto de la cuenta"
            : "Confirmar tus productos"
        }
        description={
          mode === "all"
            ? "Vas a pagar todos los productos que quedan pendientes en la mesa."
            : "Vas a pagar únicamente los productos que has seleccionado."
        }
        amount={mode === "all" ? remainingTotal : totalToPay}
